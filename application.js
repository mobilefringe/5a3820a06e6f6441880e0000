...
function renderLogoScroll(container, template, collection){
    ...
    $.each( collection , function( key, val ) {
        if(val.store_front_url !== null && !(val.store_front_url.indexOf('missing.png') > -1)){
            ...
            val.store_logo = "//codecloud.cdn.speedyrails.net/sites/5a3820a06e6f6441880e0000/image/png/1517434372411/cornwalllogoWhite@2x (1).png";
        }
        var repo_rendered = Mustache.render(template_html,val);
        item_rendered.push(repo_rendered);
    });
...
function renderPromotions(container, template, collection){
    ...
    $.each( collection , function( key, val ) {
        ...
        val.name = truncateWords(val.name, 5);
        var rendered = Mustache.render(template_html,val);
        item_rendered.push(rendered);
    });
    $(container).html(item_rendered.join(''));
    ...
    $.each(collection, function(key, val) {
        ...
        if(type == "stores" || type == "category_stores"){
            ...
            if(!val.store_front_alt_url_abs ||  val.store_front_alt_url_abs.indexOf('missing.png') > -1 || val.store_front_alt_url_abs.length === 0){
                if(!val.store_front_url_abs ||  val.store_front_url_abs.indexOf('missing.png') > -1 || val.store_front_url_abs.length === 0){
                    val.hover_img = default_image.image_url;
                } else {
